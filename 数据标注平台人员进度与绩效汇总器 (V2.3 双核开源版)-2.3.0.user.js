// ==UserScript==
// @name         数据标注平台人员进度与绩效汇总器 (V2.3 双核开源版)
...
(function () {
  ...
  async function aggregateProgress(onProgress, onDetail) {
    ...
    for (const item of allBatches) {
      ...
      // 【1. 累积折损逻辑】 (全局捕获：无论是原包还是拆分包，只要驳回就记下来)
      ...
      if (isOrigin) {
          ...
          // 质检一审
          if (qaMatchC || qaMatchE) {
              qaC = qaMatchC ? parseInt(qaMatchC[1], 10) : 0;
              qaT = true_first_total;
          } else if (reason.includes('[验收统计]') || status === 'approved' || status === 'submitted') {
              qaC = true_first_total; qaT = true_first_total;
          }

          // 验收一审
          if (accMatchC || accMatchE) {
              acc_first_C = accMatchC ? parseInt(accMatchC[1], 10) : 0;
              acc_first_T = true_first_total;
          } else if (status === 'approved') {
              acc_first_C = true_first_total; acc_first_T = true_first_total;
          }
      }
      ...
      summary.wait_label_num += (total - completed); assigneeMap[assignee].wait_label_num += (total - completed);
      summary.deliver_reviewing_num += reviewing; assigneeMap[assignee].deliver_reviewing_num += reviewing;
  ...
  const STYLES = `
    ...
    #tm-user-progress-panel { position: fixed; left: 24px; bottom: 80px; z-index: 99998; width: 1040px; max-height: 80vh; background: #fff; border-radius: 16px; box-shadow: 0 8px 40px rgba(0, 0, 0, 0.15); display: none; flex-direction: column; overflow: hidden; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; }
  ...
  function createPanel() {
    ...
    panel.innerHTML = `
      ...
      <div class="tm-user-section" id="tm-user-detail-title" style="display:none;">📋 成员明细</div>
      <div class="tm-user-table-wrap" id="tm-user-detail-wrap" style="display:none;">
        <table class="tm-user-table">
          <thead><tr><th>标注员</th><th>认领总量</th><th>🛡️质检通过率</th><th>🎯验收(首次验收通过)</th><th>🎯验收(累积折损通过)</th><th>已标注</th><th>待标注</th><th>审核中</th><th>已驳回</th><th>已完成</th></tr></thead>
  ...
  async function runAggregation() {
    ...
    try {
      ...
      summaryEl.innerHTML = `
        <div class="tm-user-card"><div class="label">📦 认领总量</div><div class="value">${formatNum(s.total_num)}</div></div>
        <div class="tm-user-card qa"><div class="label">🛡️ 质检通过率</div><div class="value">${globalQa}</div></div>
        <div class="tm-user-card"><div class="label">🎯 验收通过率(首次验收)</div><div class="value" style="color:#059669;">${globalAccFirst}</div></div>
        <div class="tm-user-card"><div class="label">🎯 验收通过率(累积折损)</div><div class="value" style="color:#d97706;">${globalAccCum}</div></div>
      ...
      if (e.target.id === 'tm-user-copy-btn') {
        ...
        rows.forEach(row => {
          ...
          if(cells[0].innerText !== '未分配') {
              ...
              const accFirstText = cells[3].innerText.replace(/\n/g, '');
              const accCumText = cells[4].innerText.replace(/\n/g, '');
              text += `- ${cells[0].innerText}: 认领${cells[1].innerText} | 质检${qaText} | 收(一审)${accFirstText} | 收(累积)${accCumText} | 完${cells[9].innerText}\n`;
          }
        });
