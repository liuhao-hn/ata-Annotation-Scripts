// ==UserScript==
// @name         数据标注平台人员进度汇总器 (固定姓名排序开源版)
// @namespace    https://github.com/yourusername
// @version      1.6.0
// @description  同时提取供应商质检与甲方验收双重通过率，适用于通用标注系统的人效数据聚合
// @author       Your Name
...
(function () {
    ...
    for (const item of allBatches) {
      ...
      if (!item.parent_batch_id) {
          ...
          if (reason.includes('[质检统计]')) {
              ...
              qaC = total; qaT = total;
              const cMatch = reason.match(/\[验收统计\].*?正确:\s*(\d+)/);
              const eMatch = reason.match(/\[验收统计\].*?错误:\s*(\d+)/);
              accC = cMatch ? parseInt(cMatch[1], 10) : 0;
              accT = accC + (eMatch ? parseInt(eMatch[1], 10) : 0);
          }
      ...
      summary.acc_correct += accC; summary.acc_total += accT;
      ...
      if (!assigneeMap[assignee]) {
        assigneeMap[assignee] = {
          ...
          total_num: 0, labeled_num: 0, wait_label_num: 0,
          deliver_reviewing_num: 0, rejected_num: 0, finished_num: 0,
          qa_correct: 0, qa_total: 0, acc_correct: 0, acc_total: 0
        };
      }
      ...
      assigneeMap[assignee].acc_correct += accC; assigneeMap[assignee].acc_total += accT;
    }

    const details = Object.values(assigneeMap).sort((a, b) => {
  ...
  const STYLES = `
    ...
    .tm-user-copy-btn { margin: 0 20px 16px; padding: 8px; border: 1px solid #ddd; border-radius: 8px; background: #fff; cursor: pointer; font-size: 13px; color: #555; text-align: center;}
  `;
  ...
  function createPanel() {
    ...
    panel.innerHTML = `
      <div class="tm-user-header"><h3>🧑‍💻 成员进度汇总 (固定姓名排序)</h3><button class="tm-user-close">✕</button></div>
