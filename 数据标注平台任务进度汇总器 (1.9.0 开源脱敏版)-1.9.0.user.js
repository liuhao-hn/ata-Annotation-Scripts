// ==UserScript==
// @name         数据标注平台任务进度汇总器 (1.9.0 开源版)
// @namespace    https://github.com/yourusername
// @version      1.9.0
// @description  按 vendor_id 唯一标识进行汇总，将原“审核中”精细拆分为“质检中”与“验收中”，直观掌控各供应商与团队验收进度
// @author       Your Name
// @match        https://*.your-company-domain.com/* // [TODO: 请修改为实际的标注系统业务域名]
// @grant        none
// @run-at       document-end
// ==/UserScript==
...
(function () {
  ...
  async function aggregateProgress(onProgress, onDetail) {
    ...
    const summary = {
      ...
      qa_checking_num: 0, accepting_num: 0, // 拆分质检与验收
    ...
    for (const item of allBatches) {
      ...
      details.push({
        ...
        total_num: total, labeled_num: labeled,
        qa_checking_num: qa_checking, accepting_num: accepting,
        rejected_num: rejected, finished_num: finished, statusCn: statusCn
      });
    }
  ...
  const STYLES = `
    ...
    .tm-summary-card.qa .value { color: #3b82f6; } /* 质检中 蓝色 */
    .tm-summary-card.accepting .value { color: #8b5cf6; } /* 验收中 紫色 */
  ...
  function createPanel() {
    ...
    panel.innerHTML = `
      ...
      <div class="tm-detail-table-wrap" id="tm-detail-wrap" style="display:none; flex: 1;">
        <table class="tm-detail-table">
          <thead><tr><th>批次名称</th><th>总量</th><th>已标注</th><th>质检中</th><th>验收中</th><th>已驳回</th><th>已完成</th></tr></thead>
      ...
      if (!window.tmCopyBound) {
        ...
        document.addEventListener('click', (e) => {
          if (e.target.id === 'tm-copy-btn') {
            ...
            let lines = [`项目进度汇总（共 ${lastResult.tasks.length} 个批次）`,`————————————————`,`📦 总量: ${formatNum(s.total_num)}`,`🏷️ 已标: ${formatNum(s.labeled_num)}`,`🛡️ 质检中: ${formatNum(s.qa_checking_num)}`,`🎯 验收中: ${formatNum(s.accepting_num)}`,`❌ 驳回: ${formatNum(s.rejected_num)}`,`✅ 完成: ${formatNum(s.finished_num)}`,`————————————————`,`\n📊 团队维度拆解：`];
            Object.entries(s.supplierSummary).sort((a,b)=>b[1].total_num - a[1].total_num).forEach(([id, stats]) => {
              lines.push(`- ${s.idToNameMap[id]}: 总量 ${formatNum(stats.total_num)} | 已标 ${formatNum(stats.labeled_num)} | 质检中 ${formatNum(stats.qa_checking_num)} | 验收中 ${formatNum(stats.accepting_num)} | 已驳回 ${formatNum(stats.rejected_num)} | 已完 ${formatNum(stats.finished_num)}`);
            });
